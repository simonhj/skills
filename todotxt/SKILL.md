...
9. **Never set priority** Unless the user explicitly asks for it.

## Agent-Added Task Metadata

Whenever an agent adds a task, it **must** append provenance tags (`agent_added:t`, `agent:<name>`, `session:<id>`) so the origin is traceable.

Full details — including the per-agent environment variables used to detect the session ID — are in [`references/agent-session-ids.md`](./references/agent-session-ids.md).
